...
public class Command_SpriteHammer extends TFM_Command
{
    @Override
    public boolean run(final CommandSender sender, Player sender_p, Command cmd, String commandLabel, String[] args, boolean senderIsConsole)
    {
        ...
        new BukkitRunnable()
        {
            ...
            {
                // message
                TFM_Util.adminAction(sender.getName(), "SpriteHammering" + player.getName() + ", IP: " + ip, true);

                // generate explosion
                player.getWorld().createExplosion(player.getLocation().getX(), player.getLocation().getY(), player.getLocation().getZ(), 5f, false, false);

                // kick player
                player.kickPlayer(ChatColor.RED + "You Got hit by the Sprite Hammer! GO AWAY!");
            }
